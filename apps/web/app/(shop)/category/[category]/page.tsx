...
async function getByCategory(category: string): Promise<Product[]> {
  ...
  const res = await fetch(`${base}/products/category/${encodeURIComponent(category)}`, { cache: 'no-store' });
  if (!res.ok) return [];
  return res.json();
}
  ...
  return (
    <main className="container">
      <div className="card p-4">
        ...
        <p className="text-muted mt-2">Products in {category}</p>
      </div>
      ...
      <div className="mt-4 grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 14 }}>
        {products.map((p) => (
          <div key={p.id} className="card p-4">
            ...
            <div className="text-muted mt-1">₹{(p.price / 100).toFixed(2)}</div>
